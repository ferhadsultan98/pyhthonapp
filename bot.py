...
async def handle_instagram_link(update: Update, context: CallbackContext):
    ...
    if 'instagram.com' in url:
        ...
        if media_type == "photo":
            # Fotoğraf ise önce mesaj gönder, sonra fotoğrafı gönder
            await update.message.reply_text("Şəklinizi tezliklə sizə təqdim edirəm.")
            await update.message.reply_photo(media_url)
        elif media_type == "video":
            # Video ise önce mesaj gönder, sonra video'yu gönder
            await update.message.reply_text("Videonuzu tezliklə sizə təqdim edirəm.")
            await update.message.reply_video(media_url)
        else:
            # Medya tipi tanınmadıysa, önce hata mesajı gönder
            await update.message.reply_text("Medya türü tespit edilemedi. Linki tekrar kontrol edin.")
            await update.message.reply_text("Doğru link göndərdiyindən əminsən dostum?")
    else:
        # Link geçerli değilse
        await update.message.reply_text("Zəhmət olmasa doğru Instagram linki göndərin.")
...
# Telegram botu çalıştırma fonksiyonu
def run_telegram_bot():
    ...
    # Botu başlat (asyncio ile)
    asyncio.run(application.run_polling(drop_pending_updates=True))

# Flask'ı ve Telegram botunu aynı anda çalıştırma
def main():
